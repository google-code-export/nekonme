...
class InstallerBase 
{
   ...
   private function generateContext():Void 
   {
      ...
      context.HXML_PATH = NME + "/tools/command-line-simple/" + target + "/hxml/" + (debug ? "debug" : "release") + ".hxml";
   }
   ...
   private function parseSsl(element:Fast):Void 
   {
      ...
      var path:String = element.has.path ? element.att.path : NME + "/tools/command-line-simple/resources/cacert.pem";
