...
class GenerateJavaExterns
{
	...
	private var mExactTypes:Hash<Bool>;
	...
	private function generate(inClass:String, inMembers:Hash<String>)
	{
		Lib.println(inClass);
	...
	private function parse(src:Input, inMembers:Hash<String>)
	{
		src.bigEndian = true;
		...
		if (is_interface)
		{
			...
			output("   public function new() { __jobject = nme.JNI.createInterface(this,\"" + dir_parts.join(".") + "." + impl_name + "\", classDef ); }\n	\n" );
		}
		...
		for (i in 0...method_count)
		{
			...
			if (expose)
			{
				...
				else
				{
					...
					output("			" + func_name + " = nme.JNI." + (is_static?"createStaticMethod":"createMemberMethod"));
		...
		if (java_out != null)
		{
			...
			Sys.command("javac", [ "-cp", "classes/android.jar",  "-sourcepath", nme_path, "-d", "compiled", "stubs/" + java_name ]);
	...
	public static function main()
	{
		...
		new GenerateJavaExterns(args[0], "gen");
	}
